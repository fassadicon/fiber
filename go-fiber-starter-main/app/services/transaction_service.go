...
func (s *TransactionService) Detail(ctx *fiber.Ctx, id int) error {
...
func (s *TransactionService) Update(ctx *fiber.Ctx, id int, req dto.TransactionRequestDTO) error {
	if err := req.Validate(); err != nil {
		return utils.JsonErrorValidation(ctx, err)
	}
	...
	if req.IsReturning != nil {
		store.IsReturning = *req.IsReturning
	}

	updated, err := s.repo().UpdateByID(repository.DB, id, store)
	if err != nil {
		return utils.JsonErrorInternal(ctx, err, "E_TRANSACTION_UPDATE")
	}

	return utils.JsonSuccess(ctx, transformer.TransactionTransformer(&updated))
}

func (s *TransactionService) Delete(ctx *fiber.Ctx, id int) error {
	if err := s.repo().DeleteByID(id); err != nil {
		return utils.JsonErrorInternal(ctx, err, "E_TRANSACTION_DELETE")
	}
