...
func argsListener() {
	for _, arg := range os.Args {
		if arg == "--rollback" {
			// Run migrations using the sql-migrate Go library instead of the external CLI.
			...
			if _, err := migrate.ExecMax(sqlDB, "postgres", migrations, migrate.Down, 0); err != nil {
				log.Fatal(err)
			}

			utils.Logger.Info("✅ up all migration")
			if _, err := migrate.ExecMax(sqlDB, "postgres", migrations, migrate.Up, 0); err != nil {
				log.Fatal(err)
			}

			// The original implementation used the external `sql-migrate` CLI:
