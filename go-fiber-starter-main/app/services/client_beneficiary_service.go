...
func (s *ClientBeneficiaryService) Delete(ctx *fiber.Ctx, clientID int, beneficiaryID int) error {
	if err := s.repo().DeleteByComposite(clientID, beneficiaryID); err != nil {
		return utils.JsonErrorInternal(ctx, err, "E_CLIENT_BENEFICIARY_DELETE")
	}

	return utils.JsonSuccess(ctx, fiber.Map{"client_id": clientID, "beneficiary_id": beneficiaryID})
}
